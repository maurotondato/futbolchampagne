...
export function ShareFormationButton({
  ...
  async function handleNativeShare() {
    ...
    try {
      ...
      const file = new File([blob], `futbol-champagne-${match.date}.png`, { type: "image/png" });
    ...
    window.open(
      whatsappLink(
        `¡Formaciones listas! ${match.teamAName} vs ${match.teamBName}. Descargá la imagen y mandala al grupo 🍾⚽`
      ),
      "_blank"
    );
  }
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", left: -99999, top: 0, pointerEvents: "none" }} aria-hidden>
        <ShareGraphic ref={nodeRef} match={match} players={players} />
      </div>
            ...
            >
              ...
              {loading && (
                <div className="flex h-80 flex-col items-center justify-center gap-3 text-ink-dim">
                  ...
                  <p className="font-hud text-xs uppercase tracking-wider">
                    Generando gráfica…
                  </p>
                </div>
              )}
              ...
              {imgUrl && !loading && (
                <>
                  ...
                  <img src={imgUrl} alt="Formación" className="w-full rounded-xl border border-line" />
